...
BEGIN_MESSAGE_MAP(CClientDemoDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BTN_DEV_CONFIG, OnBtnDevConfig)
...
BOOL CClientDemoDlg::OnInitDialog()
{
	...
	moveRect.top += nBtnHeigth;
	moveRect.bottom += nBtnHeigth;
	GetDlgItem(IDC_BTN_DEV_CONFIG)->MoveWindow(moveRect);

	tmpRect = moveRect;
	tmpRect.left += nBtnWidth;
	tmpRect.right += nBtnWidth;
	GetDlgItem(IDC_BTN_DEV_KEYBOARD)->MoveWindow(tmpRect);

	moveRect.top += nBtnHeigth;
	moveRect.bottom += nBtnHeigth;
	GetDlgItem(IDC_BTN_NET_ALARM)->MoveWindow(moveRect);

	tmpRect = moveRect;
	tmpRect.left += nBtnWidth;
	tmpRect.right += nBtnWidth;
	GetDlgItem(IDC_CHECK_OSD)->MoveWindow(tmpRect);
