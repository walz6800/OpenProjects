...
//                    "\nall variations");
  ...
  GUI_Clear();						//清楚为所设置的背景颜色
  GUI_SetColor(GUI_WHITE);			//字体颜色为白色
  GUI_SetFont(&GUI_Font8x16);		//字体大小为8*16
  GUI_DispStringHCenterAt("ECG System Loading ...", XMid, YMid - 40);		//在中间位置显示字符串
  /* Create `em */  
...
//  GUIDEMO_Delay (1000);
  ...
  for (iRep = 0; iRep < MaxReps; iRep++) {		  //进度条循环MaxReps=2 次
    ...
    PROGBAR_SetText(ahProgBar[0], "Loading ...");
